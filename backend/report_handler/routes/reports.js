...
router.get('/:id', (req, res) => {
    ...
    if (id === "" || id === undefined) {
        ...
        Reports.findOne({ "_id": id})
            .then(report => {
                if (report === null) {
                    res.status(400).send(`Error finding report ${id}.\nReport ${id} does not exist.`);
                }
                else {
                    res.send(report);
                }
            })
            .catch(err => {res.status(500).send(`Error retrieving report ${id}.\n${err}`)});
    }
...
router.get('/user/:userId', (req, res) => {
    ...
    if (userID === undefined || userID === '') {
        ...
        Reports.countDocuments({ 'userId': userID}).exec((err, count) => {
            if (err) {
                res.status(500).send(`Error finding reports filed by user ${userID}.\n${err}`);
            }
            else if (count === 0) {
                res.status(400).send(`Error finding reports filed by user ${userID}.\nUser ${userID} has not filed any reports.`);
            }
            else {
                Reports.find({ 'userId': userID})
                .then(reports => res.send(reports))
                .catch(error => res.status(500).send(`Error finding reports filed by user ${userID}.\n${error}`));
            }
        })
...
router.delete('/:id', (req, res) => {
    ...
    Reports.deleteOne({"_id": id}, (err, result) => {
        if (err) {
...
router.put('/status/:id', (req, res) => {
    ...
    .then(report => {
        if (report === null) {
            ...
            Reports.updateOne({ "_id": id}, { "status": report.status})
            .then(result => {
                if (result.n == 0) {
                    res.status(400).send(`Error updating report ${id}.\nReport ${id} not found.`);
                }
            ...
            .catch(err => res.status(500).send(`Could not update report ${id}.\n${err}`));
        }
    })
    .catch(err => res.status(500).send(`Could not update report ${id}.\n${err}`));
});
